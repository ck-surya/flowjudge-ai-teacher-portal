...
import { getDashboardStats, getTeacher, listClasses } from '@/lib/teacherService'

export default function Dashboard() {
  const [classId, setClassId] = useState('')
  const classes = useRequest(listClasses)
  const { data, loading, error, retry } = useRequest(async () => {
    const [teacher, stats] = await Promise.all([getTeacher(), getDashboardStats(classId || undefined)])
    ...
    >
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <StatCard title="Assigned Modules" value={String(stats.activeModules)} icon={Activity} color="green" href="/classes" />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {Object.entries(stats.evaluation).map(([status, count]) => <div key={status} className="bg-card border border-border rounded-lg p-4">
            ...
            {status !== 'processing' && <Link href={`/submissions?${new URLSearchParams({ status: status.toUpperCase(), ...(classId ? { classId } : {}) })}`} className="text-sm text-primary">View submissions</Link>}
          </div>)}
        </div>
